...
v0.26.1. The supported production data plane remains upstream `codexbar` CLI
...
- Upstream CodexBar CLI target: v0.26.1.
...
v0.2.0 normalizes the upstream v0.26.1 source labels used by current CLI
payloads:

- `codex-cli`, `claude`, `cli`, and `local` become semantic `source: "local"`.
- `openai-web` and `web` become semantic `source: "web"`.
- `oauth`, `oauth-api`, and `api` become semantic `source: "api"`.
...
Upstream CodexBar v0.26.1 adds `codexbar serve`, a localhost HTTP adapter for
upstream usage and cost JSON. CodexBar GNOME does not use that command; the
Shell UI remains D-Bus-only.
...
The committed upstream CLI fixture corpus now includes doc-derived v0.25.1 and
v0.26.1 compatibility fixtures for:
...
part of the corpus. Live v0.26.1 capture is optional and must stay outside
normal CI.

## Optional Live Smoke

Operators can run these commands against a local upstream v0.26.1 binary:
