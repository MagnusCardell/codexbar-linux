...
the CLI. Current development targets only the latest published upstream release
observed on 2026-06-12: CodexBar CLI v0.33.0. Older upstream versions,
including the v0.26.1 line used by the v0.2.0 cleanup work, are historical
evidence rather than a runtime compatibility goal for this personal Linux
toolbar.
...
by this project. Upstream v0.33.0 includes `codexbar serve`, but this project
does not use it because the Shell UI remains D-Bus-only.

## Optional v0.33.0 Capture

Normal CI does not require a live upstream v0.33.0 binary. Operators who need
...
CODEXBAR_CAPTURE_LIVE=1 CODEXBAR_CLI=/path/to/codexbar \
  ...
  --output /tmp/codexbar-upstream-cli-v0330 \
  --allow-provider-network \
  --providers codex,claude \
  --provider-source cli \
  --include-config-validate

./scripts/validate-upstream-cli-capture.sh /tmp/codexbar-upstream-cli-v0330
```
