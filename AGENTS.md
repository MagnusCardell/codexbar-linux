...
Build `codexbar-linux`: a native Ubuntu/GNOME top-bar companion for upstream CodexBar. The product is a GNOME Shell extension backed by a user-scoped daemon. Upstream `codexbar` CLI and local provider tooling are the supported production data plane.
...
- Browser cookies, browser profiles, provider dashboards, and desktop keyrings are out of scope. Do not read browser stores, decrypt session material, scrape provider web pages, or add a browser extension or localhost bridge.
...
- Daemon: Rust, `tokio`, `zbus`, `serde`, `serde_json`, `time`, and focused local helpers for upstream CLI execution, cache, settings, and diagnostics.
...
- Every upstream CLI or local provider refresh must distinguish: unauthenticated, provider unavailable, parse error, upstream CLI missing, timeout, stale cache, and success.
...
- Never log raw token values, Authorization headers, session keys, or provider secrets.
...
- Treat upstream CLI and local provider outputs as untrusted input. Normalizers must fail closed and produce actionable diagnostics.
